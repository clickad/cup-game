...
(function () {
  var app = {
    ...
    levelChange: function levelChange(lev) {
      switch (lev) {
        case 1:
    ...
    checkResult: function checkResult(event) {
      ...
      if (this.status === 1) {
        if ($(event.target).find('.bean').length > 0 || $(event.target)[0] === $('.bean img')[0] || $(event.target).next()[0] === $('.bean')[0]) {
          this.status = 0;
